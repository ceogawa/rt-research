...
auto dbscan(const std::shared_ptr<const std::vector<point2d>>& data, float eps, int min_pts) -> std::vector<std::vector<size_t>>
{
    const auto adapt = adaptor<point2d>(data);
    return dbscan<2>(adapt, eps, min_pts);
}

auto dbscan(const std::shared_ptr<const std::vector<point3d>>& data, float eps, int min_pts) -> std::vector<std::vector<size_t>>
{
